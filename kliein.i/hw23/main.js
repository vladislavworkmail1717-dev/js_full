...
class CalorieCalculator {
  constructor() {
    // Внутрішнє сховище продуктів на основі Map
    this.products = new Map();
  }

  // Додавання продукту та його калорійності
  addProduct(productName, calories) {
    this.products.set(productName, calories);
  }

  // Отримання калорійності продукту за назвою
...
console.log(calculator.getProductCalories('Apple')); // 52
console.log(calculator.getProductCalories('Orange')); // 'Product not found'

calculator.removeProduct('Apple');
console.log(calculator.getProductCalories('Apple')); // 'Product not found'
...
class UniqueUsernames {
  constructor() {
    // Внутрішнє сховище унікальних імен на основі Set
    this.usernames = new Set();
  }

  // Додавання нового імені до набору
  addUser(username) {
    this.usernames.add(username);
  }

  // Перевірка наявності імені серед збережених
  exists(username) {
    return this.usernames.has(username);
  }

  // Отримання загальної кількості унікальних імен
  count() {
    return this.usernames.size;
  }
}
const userStorage = new UniqueUsernames();

userStorage.addUser('alex_99');
userStorage.addUser('olga_smith');
userStorage.addUser('alex_99'); // Повторне ім'я не додасться

console.log(userStorage.count()); // 2
console.log(userStorage.exists('olga_smith')); // true
console.log(userStorage.exists('john_doe')); // false
