...
class SkillsManager {
  ...
  /**
   * Додає нову навичку до списку після валідації.
   * @param {string} skill - Навичка для додавання.
   * @returns {string|null} Додана навичка або null, якщо валідація не пройдена.
   */
  addSkill(skill) {
    const isValidSkill = typeof skill === 'string' && skill.trim().length >= 2;
  ...
  /**
   * Повертає копію масиву всіх навичок.
   * @returns {string[]} Масив навичок.
   */
  getAllSkills() {
    return [...this.skills];
  }
...
console.log(skillsManager.addSkill('JavaScript')) // Виводить: 'JavaScript'
console.log(skillsManager.addSkill('CSS')) // Виводить: 'CSS'
console.log(skillsManager.getAllSkills()) // Виводить: ['JavaScript', 'CSS']  
 ...
 */
/**
 * Функція-конструктор для створення калькулятора дат.
  * @param {string} initialDate - Початкова дата у форматі "YYYY-MM-DD".
  */  
...
/**
 * Додає задану кількість днів до поточної дати.
...
/**
 * Віднімає задану кількість днів від поточної дати.
 * @param {number} days - Кількість днів для віднімання.
 * //
 ...
 * @returns {string} Поточна дата у форматі "YYYY-MM-DD".
 * */
